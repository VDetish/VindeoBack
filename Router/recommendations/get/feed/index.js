...
function scoreItem(x, ctx) {
  ...
  const quality =
    0.7 * (x.media_quality || 0) + 0.3 * (x.has_video ? 1 : 0);
...
function diversify(items, { maxSameCatInRow = 2, sellerWindow = 5 } = {}) {
  ...
  for (const it of items) {
    const sellerTooRecent = sellers
      .slice(-sellerWindow)
      .includes(it.seller_id);
    ...
    sellers.push(it.seller_id);
    if (sellers.length > sellerWindow) sellers.shift();
  }
  for (const it of items) {
...
export default async function (res, req) {
  // ВАЖНО: первым делом создаём/валидируем сессию (вешает onAborted)
  ...
  Promise.all([sessionPromise])
    .then(async ([session]) => {
      sess = session;
      if (res.aborted) return; // если клиент отвалился — выходим
      ...
      const min = req.getQuery("min")
        ? parseInt(req.getQuery("min"), 10)
        : null;
      const max = req.getQuery("max")
        ? parseInt(req.getQuery("max"), 10)
        : null;
      ...
      const scored = rows
        .map((r) => scoreItem(r, ctx))
        .sort((a, b) => b.score - a.score);
      const top50 = diversify(scored).slice(0, 50);

      return sendJson(res, {
        session: sess,
        json: { ok: true, items: top50 },
      });
    })
    .catch((err) => {
      ...
      sendJson(res, {
        session: sess,
        status: 500,
        json: { ok: false, error: "feed error" },
      });
    });
}